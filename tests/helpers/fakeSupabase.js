...
export function fakeSupabase({ user = { id: 'user-1' }, responses = {} } = {}) {
  ...
  function builder(table) {
    ...
    const self = {
      ...
      gte(col, val) { op.filters.push(['gte', col, val]); return self; },
      lte(col, val) { op.filters.push(['lte', col, val]); return self; },
      gt(col, val) { op.filters.push(['gt', col, val]); return self; },
      lt(col, val) { op.filters.push(['lt', col, val]); return self; },
